...
/**
 * SAMPLE DATA. The products below are placeholders with example.com affiliate
 * links so the app runs end-to-end. Replace them with real, disclosed partner
 * offers before launch. All article copy here is original, written for CreditPlum.
 */
...
async function run() {
  ...
  console.log('[seed] clearing products and articles...');
  ...
  // Promote a bootstrap admin if that account already exists.
  ...
  if (adminEmail) {
    ...
    if (promoted) console.log(`[seed] promoted ${adminEmail} to admin.`);
    else console.log(`[seed] no user ${adminEmail} yet — register that email, then re-run seed to promote.`);
  }
